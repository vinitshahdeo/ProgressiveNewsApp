const apiKey = '001acb6b583d4167ace455dabb8ef715'; // Get API key from NewsAPI.org
console.log(apiKey)
...
async function updateNewsSources() {
  const response = await fetch(`https://newsapi.org/v2/sources?apiKey=001acb6b583d4167ace455dabb8ef715`);
  const json = await response.json();
  sourceSelector.innerHTML =
    json.sources.map(source => `<option value="${source.id}">${source.name}</option>`)
      .join('\n');
}
